...
void mpu6050_init(i2c_master_dev_handle_t dev)
{
  ...
  if (i2c_read_reg(dev, MPU_REG_WHO_AM_I, &whoami, 1) == ESP_OK) {
    ESP_LOGI(TAG, "WHO_AM_I = 0x%02X", whoami);
  }

  i2c_write_reg(dev, MPU_REG_PWR_MGMT_1, 0x00); // wake up
  i2c_write_reg(dev, 0x19, 7);                  // gyro sample rate
  i2c_write_reg(dev, 0x1C, 0);                  // accel range ±2g
}
...
esp_err_t mpu6050_read_accel(i2c_master_dev_handle_t dev, mpu_vec3_t *accel)
{
  ...
  if (i2c_read_reg(dev, MPU_REG_ACCEL_XOUT_H, raw, 6) != ESP_OK)
    return ESP_FAIL;
...
esp_err_t mpu6050_read_gyro(i2c_master_dev_handle_t dev, mpu_vec3_t *gyro)
{
  ...
  if (i2c_read_reg(dev, MPU_REG_GYRO_XOUT_H, raw, 6) != ESP_OK)
    return ESP_FAIL;
